    ...
    try
    {
        ...
        *********************************************************************/
        ...
        if(sourceImage!=0)
        {
            ...
            vertexUniforms+="\
                                uniform mat4 sourceImageTextureTransformation; // Transformation from camera space to overlay texture coordinate space\n";
            vertexVaryings+="\
                                varying vec2 sourceImageTexCoord; // Texture coordinate for overlay texture\n";

            /* Add water handling code to vertex shader's main function: */
            vertexMain+="\
                                /* Transform the vertex from camera space to overlay texture coordinate space: */\n\
                                vec4 sitc=sourceImageTextureTransformation*vertexCc;\n\
                                sourceImageTexCoord=sitc.xy/sitc.w;\n\
                                \n";
        }
        if(waterTable!=0)
        {
        ...
        *********************************************************************/
        ...
        if(sourceImage!=0)
        {
            /* Declare the overlay handling functions: */
            fragmentDeclarations+="\
                                void addSourceImageColor(in vec2,inout vec4);\n";

            /* Compile the water handling shader: */
            shaders.push_back(compileFragmentShader("SurfaceAddSourceImageColor"));

            /* Call water coloring function from fragment shader's main function: */

            fragmentMain+="\
                         /* Modulate the base color with overlay color: */\n \
                         addSourceImageColor(gl_FragCoord.xy,baseColor);\n \
                         \n";
        }
        if(waterTable!=0)
        {
        ...
        *(ulPtr++)=glGetUniformLocationARB(result,"depthProjection");
        ...
        if(sourceImage!=0)
        {
            *(ulPtr++)=glGetUniformLocationARB(result,"sourceImageTextureTransformation");
            *(ulPtr++)=glGetUniformLocationARB(result,"sourceImageSampler");
            *(ulPtr++)=glGetUniformLocationARB(result,"sourceImageOpacity");
        }
        if(waterTable!=0)
        {
...
void SurfaceRenderer::initContext(GLContextData& contextData) const
{
    ...
    /* Create the global ambient height map render shader: */
    {
        ...
        // uniforms for overlay
        dataItem->globalAmbientHeightMapShaderUniforms[10]=glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader,"sourceImageSampler");
        dataItem->globalAmbientHeightMapShaderUniforms[11]=glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader,"sourceImageTextureTransformation");
        dataItem->globalAmbientHeightMapShaderUniforms[12]=glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader,"sourceImageOpacity");
    }
    ...
    /* Create the shadowed illuminated height map render shader: */
    {
        ...
        // uniforms for overlay
        dataItem->shadowedIlluminatedHeightMapShaderUniforms[13]=glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader,"sourceImageSampler");
        dataItem->shadowedIlluminatedHeightMapShaderUniforms[14]=glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader,"sourceImageTextureTransformation");
        dataItem->shadowedIlluminatedHeightMapShaderUniforms[15]=glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader,"souceImageOpacity");
    }
}
...
void SurfaceRenderer::setSourceImage(SourceImage* newSourceImage)
{
...
void SurfaceRenderer::glRenderSinglePass(GLuint heightColorMapTexture,GLContextData& contextData) const
{
    ...
    if(sourceImage!=0)
    {
        ...
        /* Upload the overlay opacity factor: */
        glUniform1fARB(*(ulPtr++),sourceImageOpacity);
